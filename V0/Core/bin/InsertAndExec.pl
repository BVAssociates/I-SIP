...
#	-------------------------------------------------------------------
#
#@I-SIS	FONCTION : Insertion dans une table avec post-action
#
#@I-SIS	USAGE :  InsertAndExec [-h] [-f] INTO <Table> VALUES <-|Values>
#
...
sub Trigger_Action
{
		...
		"Processor='ExecuteProcedure'\" 2>nul |"))
	{
		...
		system("Log_Error 0308 \"Get_PCI For $TableName\"");
		&Exit_Procedure;
	}
	@Commands = <INPUT>;
	close(INPUT);
	$RETURN_CODE = $? >> 8;
	&Check_ReturnCode("Get_PCI For $TableName");
...
#Syntaxe du script
$USAGE="USAGE: $FONCTION [-h] [-f] INTO <Table> VALUES <-|Values>";
...
# On va exécuter la commande de pré-insertion
&Trigger_Action("Insert", "PreAction");

# On va exécuter la commande d'insertion normale
system("Insert $Force INTO $TableName VALUES \"$Values\" 2>nul");
$RETURN_CODE = $? >> 8;
&Check_ReturnCode("Insert INTO $TableName");

# On va exécuter la commande de post-insertion
&Trigger_Action("Insert", "PostAction");
