...
#	-------------------------------------------------------------------
#
#@I-SIS	FONCTION : Suppression depuis une table avec post-action
#
#@I-SIS	USAGE :  RemoveAndExec [-h] FROM <Table> VALUES <-|Values>
#
#@I-SIS	OPT : "-h pour avoir l'utilisation de la ligne de commande"
#
#@I-SIS	ARG : "Table : Nom de la table depuis laquelle supprimer les valeurs"
#@I-SIS	ARG : "- : Utiliser les valeurs provenant de l'entrée standard"
#@I-SIS	ARG : "Values : Valeurs à supprimer de la table"
#
...
#Syntaxe du script
$USAGE="USAGE: $FONCTION [-h] FROM <Table> VALUES <-|Values>";
...
# On vérifie que le 1er argument vaut FROM
if(uc($ARGV[0]) ne "FROM")
{
	$BV_SEVERITE = 202;
	&Export_Variables("BV_SEVERITE");
...
&Trigger_Action("Remove", "PreAction");

# On va exécuter la commande de suppression normale
system("Remove FROM $TableName VALUES \"$Values\" 2>nul");
$RETURN_CODE = $? >> 8;
&Check_ReturnCode("Remove FROM $TableName");

# On va exécuter la commande de post-action
&Trigger_Action("Remove", "PostAction");
