...
foreach my  $table_name (@table_list) {
	...
	my $rules=$env_sip->get_isip_rules($table_name);
	...
	while (my %row=$table_status->fetch_row) {
		...
		$row{ICON}=$rules->get_field_icon($row{FIELD_NAME},$row{STATUS}, $row{COMMENT}) if exists $row{ICON} and $explore_mode eq "explore";
		...
		if (not ($row{TYPE} and $row{TYPE} eq "exclus")
				and not($row{ICON} and $row{ICON} eq "stamp")) {
