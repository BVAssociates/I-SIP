...
* Revision 1.9  2009/02/25 16:40:31  tz
* Interdiction d'avoir des " et % dans la valeur pour les Agents Windows.
	...
	extends BaseDialog
{
	...
	* ----------------------------------------------------------*/
	public void validateInput() {
		...
		agent_layer_mode = getAgentLayerMode();
		try {
			...
			// Si le mode de la couche d'exécution est Windows, on ne doit
			// pas avoir à la fois de " et de % dans la valeur
			if(agent_layer_mode.equalsIgnoreCase("WINDOWS") == true) {
				if(values.indexOf("\"") > 0 && 
					values.indexOf("%") > 0) {
					// On va afficher un message d'erreur
					getMainWindowInterface().showPopup("Error",
						"&ERR_CannotMixQuotesAndPercent", null);
					// On arrête
					values = null;
					trace_methods.endOfMethod();
					return;
