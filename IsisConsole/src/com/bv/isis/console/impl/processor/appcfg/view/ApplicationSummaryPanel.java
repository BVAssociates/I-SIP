...
* ------------------------------------------------------------
*
* $Source: /cvs/inuit/ClientHMI/src/com/bv/isis/console/impl/processor/appcfg/view/ApplicationSummaryPanel.java,v $
* $Revision: 1.2 $
*
* ------------------------------------------------------------
 ...
 * rentrées.
 * ------------------------------------------------------------*/
public class ApplicationSummaryPanel extends AbstractPanel {
	 ...
	 * ------------------------------------------------------------*/
	public ApplicationSummaryPanel(WindowInterface mainWindow) {
		super();
		
		Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"ApplicationSummaryPanel", "ApplicationSummaryPanel");
	...
	public void beforeDisplay(ArrayList<ModelInterface> modele) {
		Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"ApplicationSummaryPanel", "beforeDisplay");
			...
			try {
				...
				for (int count = 0 ; count < 4 ; count++) {
					...
					for (int index = 0 ; index < modele.size() ; index ++) {
						...
						if (count == 0 && modele.get(index) instanceof PortalApplicationServices ) {
							PortalApplicationServices c = (PortalApplicationServices) modele.get(index);
							if (c.getFlag() == 'A') {
						...
						else if (count == 1 && modele.get(index) instanceof PortalApplicationStates ) {
							PortalApplicationStates c = (PortalApplicationStates) modele.get(index);
							if (c.getFlag() == 'A') {
								tableToElementsToAdd.add(c);
							}else if (c.getFlag() == 'M') {
								tableToElementsToUpd.add(c);
						...
						}else if (count == 2 && modele.get(index) instanceof PortalApplicationGroupTransitions ) {
							PortalApplicationGroupTransitions c = (PortalApplicationGroupTransitions) modele.get(index);
							if (c.getFlag() == 'A') {
								tableToElementsToAdd.add(c);
							}else if (c.getFlag() == 'M') {
								tableToElementsToUpd.add(c);
						...
						}else if (count == 3 && modele.get(index) instanceof PortalApplicationDoc ) {
							PortalApplicationDoc c = (PortalApplicationDoc) modele.get(index);
							if (c.getFlag() == 'A') {
								tableToElementsToAdd.add(c);
							}else if (c.getFlag() == 'M') {
								tableToElementsToUpd.add(c);
	...
	public boolean afterDisplay() {
		Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"ApplicationSummaryPanel", "afterDisplay");
		trace_methods.beginningOfMethod();
		trace_methods.endOfMethod();
	...
	public ArrayList<ModelInterface> update(ArrayList<ModelInterface> tabModels) {
		Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"ApplicationSummaryPanel", "update");
		Trace trace_arguments = TraceAPI.declareTraceArguments("Console");
	...
	private void makePanel() {
		Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"ApplicationSummaryPanel", "makePanel");
		trace_methods.beginningOfMethod();
