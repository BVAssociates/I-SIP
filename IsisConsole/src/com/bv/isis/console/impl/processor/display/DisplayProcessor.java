...
* Revision 1.40  2009/03/05 09:39:43  jy
* Création d'une méthode loadMaskRules() qui petmet de récupérer des masques en appelant la méthode statique readXMLFile() de la classe ReaderXML
*
...
* Cette classe se charge aussi de modifier l'affichage en fonction d'un fichier
* XML s'il existe.
* ----------------------------------------------------------*/
	...
	implements DisplayProcessorInterface
{
	...
	* ----------------------------------------------------------*/
			...
			InnerException
	{
		...
		else
		{
			...
			{
				case 6:
					_tableMask = tokenizer.getToken(5);
				case 5:
	...
	* Elle permet aussi d'appeler la méthode loadMaskRules() qui se chargee de
	* lire un fichier du XML de description des masques d'affichage et les 
	* stocker dans le modèle (en appelant la méthode statique readXMLFile() de 
	* la classe ReaderXML et la méthode setMaskRules() de la classe 
	* DisplayDataModel).
	* 
	* Arguments:
	...
	* ----------------------------------------------------------*/
		...
		)
	{
		...
		try
		{
			...
			if(isFirstLoading == true)
			{
				// On charge des masques et les stocke dans le modèle.
				String path_xml = null;
				String temporary_directory = "temp";
				FileReaderInterface file_reader_interface = null;
				File destination_file = null;
				try {
					path_xml = session_proxy.getControlPanelFilePath(_tableMask
						+ "_mask", _nodeContext);	
					file_reader_interface = session_proxy.getFileReader(
						path_xml);
					try {
					...
					// _tableName + "_mask.xml"
					destination_file = File.createTempFile(_tableMask + "_mask",
						".xml", new File(temporary_directory));
					FileDownloader.downloadFile(file_reader_interface, 
						1024*1024, destination_file, null);
					// On lire le fichier.
					loadMaskRules(destination_file.getAbsolutePath(), 
						table_definition);
					...
					// On ne fait rien
					trace_errors.writeTrace(
						"Erreur lors du chargement du fichier de masque : " 
							+ ex.getMessage());
				}
			}
	...
	* 
	* Description:
	* Cet attribut maintient aussi le nom du fichier de masque.
	* ----------------------------------------------------------*/
	...
	* 
	* Description:
	* Cette méthode permt de charger des masques définit dans un fichier XML
	* et faire passer une référence d'un objet de IsisTableDefinition.
	* Si un problème est détecté durant la lecture de fichier, l'exception 
	* InnerException doit être levée.
	* 
	* Arguments:
	*  - path: Le chemin de fichier XML.
	...
	* ----------------------------------------------------------*/
	...
	{
			Trace trace_methods = TraceAPI.declareTraceMethods("Console",
				"DisplayProcessor", "loadMaskRules");
			Trace trace_arguments = TraceAPI.declareTraceArguments("Console");
			...
			isisTableDefinition);
			MaskRule[] mask_rules;
			mask_rules = ReaderXML.readXMLFile(path, isisTableDefinition);
			_dataModel.setMaskRules(mask_rules);
			trace_methods.endOfMethod();
	}
}